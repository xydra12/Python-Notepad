...
def notepad_exit():
    answer = messagebox.askokcancel('Выход', 'Точно?')
    if answer:
        root.destroy()
...
file_menu.add_command(label='Открыть .txt фаил', command=open_file)
file_menu.add_command(label='Сохранить .txt фаил', command=save_file)
file_menu.add_separator()
file_menu.add_command(label='Закрыть без сохранений', command=notepad_exit)
...
view_menu.add_cascade(label='Изменить тему', menu=view_menu_sub)
...
view_menu.add_cascade(label='Изменить шрифт', menu=font_menu_sub)
